...
export default function CasaShowEventosScreen() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      setError('');
      const response = await eventService.listByCasaShow(session.id_usuario);
  ...
  const summary = useMemo(() => {
    ...
    const ativos = events.filter((item) => isFutureOrToday(item.data_inicio)).length;
    const finalizados = events.filter((item) => {
      const date = parseApiDate(item.data_inicio);
      return date ? date.getTime() < startOfToday().getTime() : false;
    }).length;

    return { total, ativos, finalizados, cancelados: 0 };
  }, [events]);
